...
html = mustReplace(html, '<title>OpenShop v0.31.0 | Browser Image Editor</title>', '<title>KanPaint v0.3 | Browser Image Editor</title>', 'title');
html = mustReplace(html, '</title>\n<script>', '</title>\n<link rel="stylesheet" href="./kanpaint-v03.css">\n<script>', 'extension stylesheet');
html = mustReplace(html,
...
`    <div class="logo" aria-label="KanPaint version 0.2">
        ...
        <span class="logo-version">v0.3</span>
    </div>`,
  'logo');
html = mustReplace(html, '</script>\n</body>\n</html>', '</script>\n<script src="./kanpaint-v03.js"></script>\n</body>\n</html>', 'extension script');
write('index.html', html);

const srcDir = path.join(root, 'src');
fs.copyFileSync(path.join(srcDir, 'kanpaint-v03.js'), path.join(out, 'kanpaint-v03.js'));
fs.copyFileSync(path.join(srcDir, 'kanpaint-v03.css'), path.join(out, 'kanpaint-v03.css'));
...
if (fs.existsSync(runtimePath)) {
  ...
  if (!runtime.includes("'./kanpaint-v03.js'")) {
    runtime = mustReplace(runtime, anchor, `${anchor}  './kanpaint-v03.js',\n  './kanpaint-v03.css',\n`, 'runtime asset list');
  }
...
if (fs.existsSync(swPath)) {
  ...
  sw = mustReplace(sw, "const SHELL_REVISION = '0.31.0-r1';", "const SHELL_REVISION = '0.3.0-r1';", 'service worker revision');
  ...
  if (!sw.includes('"./kanpaint-v03.js"'))
    sw = mustReplace(sw, assetAnchor, `${assetAnchor}    "./kanpaint-v03.js",\n    "./kanpaint-v03.css",\n`, 'service worker assets');
...
console.log(`KanPaint v0.3 built at ${out}`);
