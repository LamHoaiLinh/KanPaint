...
`  skin:Object.freeze({info:()=>request('skin.info'),preset:(name)=>request('skin.preset',{name}),layers:()=>request('skin.layers'),editActive:()=>request('skin.editActive')}),\n` +
...
`(async()=>{ try {\n${source}\n; window.parent.postMessage({type:'kanpaint:script-complete',pluginId:host.pluginId,token:host.token,ok:true},'*'); } catch(error) { window.parent.postMessage({type:'kanpaint:script-complete',pluginId:host.pluginId,token:host.token,ok:false,error:String(error?.message||error)},'*'); } })();`;
    ...
    // ---------------------------------------------------------------------
    // KanPaint v0.4 non-destructive Skin Retouch layers.
    // ---------------------------------------------------------------------
    ...
    Skin._renderRetouchOverlay = function() {
        ...
        for(let i=0;i<s.mask.length;i++){
            const m=s.mask[i]/255;if(m<=0)continue;const p=i*4,safeM=Math.max(m,1/255);
    ...
    Skin._retouchMeta = function(target) {
        const s=this.session;return {format:'kanpaint-skin-retouch',version:1,targetObjectId:OS._ensureObjectId(target),
            params:{...this.params},maskDataUrl:this._maskToDataUrl(s.mask,s.source.width,s.source.height),
    ...
    Skin.loadActiveRetouch = async function(active=null,{silent=false}={}) {
        ...
        this.params={...this.params,...(meta.params||{})};this.outputMode='layer';
    ...
    Skin.onDown=function(opt){if(this.session?.editingObject){const ptr=OS.canvas.getScenePoint(opt.e);this.painting=true;this._paint(ptr);return;}return _skinOnDownV03(opt);};
    ...
    function addSkinUI() {
        ...
        const panel=el('aside',{id:'kp-skin-panel'}),head=el('div',{class:'kp-panel-head'},[el('span',{text:'Skin Retouch'}),el('small',{text:'KanPaint v0.4'})]),body=el('div',{class:'kp-panel-body'});
        ...
        const status=el('div',{id:'kp-skin-status',class:'kp-progress',text:'Paint a mask or use the current selection.'});body.append(status,el('p',{class:'kp-help',text:'Default output is a separate editable Retouch Layer. The original image stays unchanged. Select that Retouch Layer later and choose Skin Retouch to edit it again.'}));
        const maskActions=el('div',{class:'kp-panel-actions'}),useSelection=button('Use Selection'),clearMask=button('Clear Mask'),editActive=button('Edit Active Retouch');editActive.id='kp-skin-edit-active';maskActions.append(useSelection,clearMask,editActive);body.append(maskActions);
        const actions=el('div',{class:'kp-panel-actions'}),before=button('Hold: Before'),pApply=button('Apply','btn btn-primary'),pCancel=button('Cancel');actions.append(before,pApply,pCancel);body.append(actions);panel.append(head,body);document.body.append(panel);
        preset.addEventListener('change',()=>Skin.applyPreset(preset.value));useSelection.addEventListener('click',()=>void Skin.fillSelection());clearMask.addEventListener('click',()=>Skin.clearMask());editActive.addEventListener('click',()=>void Skin.loadActiveRetouch());
    ...
    function brandVisibleShell() {
        const logo=document.querySelector('.logo'); if(logo){logo.setAttribute('aria-label','KanPaint version 0.2');const mark=logo.querySelector('.logo-mark');const word=logo.querySelector('.logo-word');const ver=logo.querySelector('.logo-version');if(mark)mark.textContent='KP';if(word)word.textContent='KanPaint';if(ver)ver.textContent='v0.4';}
        document.title='KanPaint v0.4 | Browser Image Editor';
    }
