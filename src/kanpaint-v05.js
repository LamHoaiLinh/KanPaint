...
`(async()=>{ try {\n${source}\n; window.parent.postMessage({type:'kanpaint:script-complete',pluginId:host.pluginId,token:host.token,ok:true},'*'); } catch(error) { window.parent.postMessage({type:'kanpaint:script-complete',pluginId:host.pluginId,token:host.token,ok:false,error:String(error?.message||error)},'*'); } })();`;
    ...
    // ---------------------------------------------------------------------
    // KanPaint v0.5 non-destructive Skin Retouch layers.
    // ---------------------------------------------------------------------
    ...
    function addSkinUI() {
        ...
        const panel=el('aside',{id:'kp-skin-panel'}),head=el('div',{class:'kp-panel-head'},[el('span',{text:'Skin Retouch'}),el('small',{text:'KanPaint v0.5'})]),body=el('div',{class:'kp-panel-body'});
    ...
    function brandVisibleShell() {
        const logo=document.querySelector('.logo'); if(logo){logo.setAttribute('aria-label','KanPaint version 0.5');const mark=logo.querySelector('.logo-mark');const word=logo.querySelector('.logo-word');const ver=logo.querySelector('.logo-version');if(mark)mark.textContent='KP';if(word)word.textContent='KanPaint';if(ver)ver.textContent='v0.5';}
        document.title='KanPaint v0.5 | Browser Image Editor';
    }
